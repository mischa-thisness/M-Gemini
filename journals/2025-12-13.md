...
* **Key Architectural Decisions:**
    *   Limited SSH interaction capability: Can initiate SSH connections via `run_shell_command`, but cannot programmatically interact within the session. User interaction is required via `Ctrl + F`.
*   **Critical Bugs Identified/Fixed:**
    *   Identified limitation of non-interactive `run_shell_command` preventing automated tasks within SSH sessions.
*   **New Features/Scripts Implemented:**
    *   Implemented SSH command execution functionality, leveraging `run_shell_command`.
    *   Added user guidance for potential password prompts/key verifications during SSH connection.
