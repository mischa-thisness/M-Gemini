...
* **Key Architectural Decisions:**
    * Adopted PowerShell scripts for system analysis due to CLI limitations.
    * Prioritized a streamlined service report focusing on running services and associated details.
    * Correlated process IDs from service inventory to CPU and memory usage using `Get-Process`.
* **Critical Bugs Identified/Fixed:**
    * No explicit bugs were fixed, but the analysis of Mullvad VPN startup entries aimed at identifying potential misconfigurations or issues.
* **New Features/Scripts Implemented:**
    * Script to list and analyze system startup applications, focusing on Mullvad VPN.
    * Script to review services, including inventory, status, resource usage (CPU, memory), dependencies, and error logs.
* **Specific Tools and Libraries Utilized:**
    * PowerShell: `Get-CimInstance`, `Get-Service`, `Get-Process`, `Get-EventLog`
